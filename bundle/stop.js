...
async function enrichSkillRuns(options) {
  ...
  for (const enrichment of skillEnrichments) {
    try {
      ...
      await runTree.patchRun({ excludeInputs: true });
      debug(`Enriched Skill run ${enrichment.runId} with content for ${enrichment.skillName}`);
    } catch (err) {
      error(`Failed to enrich Skill run ${enrichment.runId}: ${err}`);
    }
  }
}
      ...
      for (const tc of llmCall.toolCalls) {
        if (tc.skillContent && freshSession.skill_tool_runs?.[tc.tool_use.id]) {
          ...
          skillEnrichments.push({
            ...
            skillName: tc.tool_use.input.skill ?? tc.tool_use.name
          });
        }
      }
    }
